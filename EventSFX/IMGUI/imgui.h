...
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Data to shared draw data (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)
    const char*             _OwnerName;         // Data to owner window's name for debugging
...
struct ImFontConfig
{
    ...
    const ImWchar*  GlyphRanges;            // NULL     // Data to a user-provided list of Unicode range (2 value per range, values are inclusive, zero-terminated list). THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE.
...
struct ImFont
{
    ...
    const ImFontConfig*         ConfigData;         // 4-8   // in  //            // Data within ContainerAtlas->ConfigData
