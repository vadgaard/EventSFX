...
{
	globalCVarManager->log(std::vformat(format_str, std::make_format_args(std::forward<Args>(args)...)));
}

template <typename... Args>
void LOG(std::wstring_view format_str, Args&&... args)
{
	globalCVarManager->log(std::vformat(format_str, std::make_wformat_args(std::forward<Args>(args)...)));
}
...
void DEBUGLOG(const FormatString& format_str, Args&&... args)
{
	if constexpr (DEBUG_LOG)
	{
		...
		globalCVarManager->log(std::format("{} {}", text, location));
	}
}
...
void DEBUGLOG(const FormatWstring& format_str, Args&&... args)
{
	if constexpr (DEBUG_LOG)
	{
		...
		auto location = format_str.GetLocation();
		globalCVarManager->log(std::format(L"{} {}", text, location));
	}
}
